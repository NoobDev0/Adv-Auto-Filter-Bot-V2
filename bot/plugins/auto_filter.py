...
async def auto_filter(bot, update):
    ...
    if len(results) == 0: # double check
        ...
        try: 
            ...
            deco=dict["image"]
            if dict:
                info=f"🎬 <b>Movie/Series</b> : <code>{titlee}</code> /n🔥 <b>Released</b> : <code>{yearr}</code> /n💫 <b>Rating</b> : <code>{ratingg}</code> /n🎭 <b>Genre</b> : <code>{genree}</code> /n <u>@Cinema_Haunter</u>"
            else:
